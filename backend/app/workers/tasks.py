...
def process_document(self, job_id: str):
    logger.info(f"🔍 [Task DEBUG] process_document starting for job_id={job_id}")
    db = get_sync_db()
    logger.info(f"🔍 [Task DEBUG] Database session created.")

    try:
        logger.info(f"🔍 [Task DEBUG] Fetching job {job_id}...")
        job = db.query(DocumentJob).filter(DocumentJob.id == job_id).first()
        if not job:
            logger.error(f"❌ [Task DEBUG] Job {job_id} not found.")
            return {"error": "Job not found"}

        logger.info(f"🔍 [Task DEBUG] Updating job ID and status...")
        ...
        logger.info(f"✅ [Task DEBUG] Job status updated to processing.")

        publish_progress(job_id, "job_started", "in_progress", "Worker picked up the job.")
        
        logger.info(f"🔍 [Task DEBUG] Sending to Gemini...")
        ...
        logger.info(f"✅ [Task DEBUG] Gemini call finished.")
